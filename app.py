...
async function signup(){
  ...
  // Upload profile pic directly to Cloudinary (if provided)
  let profilePicUrl = '';
  const pic = byId('signupPic').files[0];
  if(pic){
    try {
      profilePicUrl = await uploadFile(pic, 'vibenet/avatars');
    } catch(e) {
      console.warn('Profile pic upload failed:', e);
    }
  }

  // Submit as JSON — no file in this request, pic URL already resolved
...
def api_signup():
    # Accepts JSON — profile pic is uploaded directly to Cloudinary by the browser
    ...
    profile_pic = data.get("profile_pic", "")  # URL already uploaded by browser
