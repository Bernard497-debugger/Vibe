...
async function uploadFile(file, folder='vibenet/posts'){
  // Try direct browser → Cloudinary (fast, no server middleman)
  try {
    ...
    if(sigRes.ok){
      ...
      if(sig.cloud_name && sig.api_key){
        ...
        const cldRes = await fetch(`https://api.cloudinary.com/v1_1/${sig.cloud_name}/auto/upload`, {method:'POST', body:fd});
        if(cldRes.ok){
          const cld = await cldRes.json();
          return (cld.eager && cld.eager[0] && cld.eager[0].secure_url) || cld.secure_url || '';
        }
      }
    }
  } catch(e) {}
  // Fallback: server upload (local dev)
