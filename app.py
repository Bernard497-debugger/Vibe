      ...
      <div id="profile" class="tab">
        ...
        <div class="profile-header">
          ...
          <div class="profile-info">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px\">\n              <div class="profile-name" id="profileName">—</div>\n              <span id="profileVerified" style="display:none;color:var(--accent);font-size:13px;font-weight:700\">✦ Verified</span>\n            </div>
            <div class="profile-email" id="profileEmail">—</div>
            <div style="margin-top:14px\">\n              <label style="font-size:12px;color:var(--muted2);font-weight:500;text-transform:uppercase;letter-spacing:0.3px;display:block;margin-bottom:8px\">Bio</label>
...
function onLogin(){
  ...
  const verifyBadge = byId('profileVerified');
  if(currentUser.verified){ verifyBadge.style.display = 'inline'; } else { verifyBadge.style.display = 'none'; }
...
async function loadNotifications(markSeen=false){
  ...
  list.forEach(n=>{
    ...
    d.onclick = async () => {
      if(!n.seen) {
        await fetch(API+'/notifications/'+n.id+'/mark-read', {method:'POST'});
        d.style.background = '';
        n.seen = 1;
      }
    };
