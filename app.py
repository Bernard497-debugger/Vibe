...
function createAdCard(ad){
  ...
  div.style.cssText = 'background:linear-gradient(135deg,rgba(77,240,192,0.06),rgba(0,201,255,0.04));border:1px solid rgba(77,240,192,0.2);border-radius:16px;padding:16px 18px;margin-bottom:12px;position:relative';
  div.innerHTML = `
    ...
    <div style="font-size:15px;font-weight:700;color:#e8f0ff;margin-bottom:4px">${escapeHtml(ad.title||'')}</div>
    <div style="font-size:12px;color:#5a6a85">Promoted on VibeNet</div>
  `;
  return div;
}
...
async function loadFeed(){
  ...
  list.forEach((p, i) => {
    ...
    // Inject an ad card every 5 posts if ads available
    if(ads.length && (i+1) % 5 === 0 && adIndex < ads.length){
