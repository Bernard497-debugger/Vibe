...
def api_ads():
    if request.method == "POST":
        data = request.get_json() or {}
        ad   = Ad(title=data.get("title"), owner_email=data.get("owner"), budget=data.get("budget", 0), approved=0)
        db.session.add(ad)
        db.session.commit()
...
def admin_page():
    if not require_admin():
        return "<h2 style='font-family:sans-serif;color:red;padding:40px'>403 — Unauthorized</h2>", 403
    ...
    def badge(val, labels=("Pending","Approved","Rejected"), colors=("f0c84d","4DF0C0","f06a4d")):
        ...
        hex_col = colors[val]
        label = labels[val]
        return f'<span style="background:rgba({rgb},0.15);color:#{hex_col};padding:3px 10px;border-radius:100px;font-size:11px;font-weight:700">{label}</span>'
    ...
    rows_ads = "".join(f"""<tr>
      ...
      </td></tr>""" for a in ads) if ads else f'<tr><td colspan="7" style="color:#5a6a85;text-align:center;padding:20px">No campaigns yet</td></tr>'
    ...
    </tr>""" for p in payouts) if payouts else f'<tr><td colspan="6" style="color:#5a6a85;text-align:center;padding:20px">No payout requests</td></tr>'

    rows_users = "".join(f"""<tr>
      <td style="font-weight:600">{u.name or '—'}</td>
      <td style="color:#8899b4;font-size:12px">{u.email}</td>
      <td>{Follower.query.filter_by(user_email=u.email).count()}</td>
      <td>{u.watch_hours}</td>
      <td style="color:#4DF0C0">P{u.earnings:.2f}</td>
      <td style="color:#5a6a85;font-size:12px">{u.created_at}</td>
    </tr>""" for u in users) if users else f'<tr><td colspan="6" style="color:#5a6a85;text-align:center;padding:20px">No users yet</td></tr>'
...
th{{text-align:left;color:#5a6a85;font-weight:600;padding:8px 10px;border-bottom:1px solid rgba(255,255,255,0.06)}}
td{{padding:10px;border-bottom:1px solid rgba(255,255,255,0.04);vertical-align:middle}}
tr:last-child td{{border-bottom:none}}
button{{border:none;padding:7px 14px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer}}
...
<div class="section">
  <h2>👥 Users</h2>
  <table><tr><th>Name</th><th>Email</th><th>Followers</th><th>Watch Hrs</th><th>Earnings</th><th>Joined</th></tr>
  {rows_users}</table>
</div>
