...
function createAdCard(ad){
  ...
  div.innerHTML = `
    ...
    ${waLink ? `<a href="${waLink}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#fff;font-size:13px;font-weight:700;padding:9px 16px;border-radius:100px;text-decoration:none">
      ...
      Chat on WhatsApp
    </a>` : '<div style="font-size:12px;color:#5a6a85">Promoted on VibeNet</div>'}
  `;
  return div;
}
...
async function loadFeed(){
  ...
  let adIndex = 0;
  // Show first ad at top of feed immediately
  if(ads.length) feed.appendChild(createAdCard(ads[0]));
  list.forEach((p, i) => {
