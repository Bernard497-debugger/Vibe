...
def upload_to_cloudinary(file_storage, folder="vibenet"):
    """Upload a FileStorage object to Cloudinary. Returns public URL or raises.
    Used only for profile pic uploads during signup (small images).
    Post media uses direct browser→Cloudinary upload via /api/sign-upload."""
...
def _optimize_cloudinary_url(url, is_video=False):
    """Insert transformation params into a Cloudinary URL for fast delivery."""
    if not url or "cloudinary.com" not in url:
        return url
    if is_video:
        # q_auto: auto quality, f_auto: best format (e.g. webm for Chrome),
        # sp_auto: adaptive streaming profile, vc_auto: auto video codec
        transforms = "q_auto,f_auto,vc_auto"
...
// Upload file: tries direct browser→Cloudinary first (fast),
// falls back to server relay if Cloudinary not configured.
async function uploadFile(file){
  // 1. Ask server for a signed upload ticket
  let sigRes;
  try {
    sigRes = await fetch(API + '/sign-upload', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ folder: 'vibenet/posts' })
    });
  } catch(e) { sigRes = null; }
  ...
  if(sigRes && sigRes.ok){
    ...
    if(sig.cloud_name && sig.api_key){
      ...
      // Ask Cloudinary to auto-optimize on delivery
      fd.append('eager', 'q_auto,f_auto,vc_auto');
      ...
      if(cldRes.ok){
        ...
        // Return the eager (optimized) URL if available, else secure_url
        const url = (cld.eager && cld.eager[0] && cld.eager[0].secure_url)
                    || cld.secure_url || '';
        return url;
      }
    }
  ...
  // Fallback: server-side upload (local dev / no Cloudinary)
  const fd = new FormData();
  fd.append('file', file);
...
// Rewrite a Cloudinary URL to insert quality/format optimizations
function optimizeCldUrl(url, isVideo){
  if(!url || !url.includes('cloudinary.com')) return url;
  const t = isVideo ? 'q_auto,f_auto,vc_auto' : 'q_auto,f_auto';
...
def api_upload():
    """Fallback server-side upload — only used if Cloudinary not configured."""
...
def api_sign_upload():
    """Return a Cloudinary signed upload params so the browser uploads
    directly to Cloudinary — no server middleman for large files."""
