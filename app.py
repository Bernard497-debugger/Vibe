...
def admin_page():
    ...
    for u in users:
        ...
        reactions_total= db.session.query(func.sum(UserReaction.id)).filter(
            UserReaction.post_id.in_(
                db.session.query(Post.id).filter_by(author_email=u.email)
            )
        ).scalar() or 0
        ...
        rows_users += f"""<tr>
          ...
          <td>
            <div style="display:flex;gap:5px;flex-wrap:wrap">
              ...
              <button onclick=\'deleteUser("{u.email}")\' style=\'{BTN_RED}\'>🗑 Delete</button>
            </div>
          </td>
        </tr>"""
